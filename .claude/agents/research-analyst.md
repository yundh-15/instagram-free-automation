...
## 호출 조건 (토큰 효율) — 가장 비싼 에이전트, 캐시 우선
- **저빈도로만** 호출(시간/일 단위). 매 사이클 호출 절대 금지.
- 브리지 파일(`data/coin-signals/research-<SYMBOL>.json`)이 **TTL 내로 신선하면 재호출하지 않고 캐시 사용**.
- 재호출은 ① TTL 만료, 또는 ② 중대한 뉴스/이벤트(규제·해킹·상장 등) 발생 시에만.
