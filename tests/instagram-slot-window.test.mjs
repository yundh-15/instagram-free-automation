...
test('late publication is observed as part of the same slot until the next slot begins', () => {
  const slot = parseSlot('2026-05-25T17');

  assert.equal(slotPublishCutoffUtc(slot).toISOString(), '2026-05-25T10:00:00.000Z');
...
test('current slot switches at the next Korean scheduled hour', () => {
  assert.equal(currentSlot(new Date('2026-05-25T12:57:32Z')).key, '2026-05-25T17');
...
test('scheduled workflow does not opt into publishing after the cutoff', async () => {
  ...
  assert.match(workflow, /npm run run:instagram-slot -- --fallback-publish/);
